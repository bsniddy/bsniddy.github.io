...
const PROJECTS = [
  ...
  /* ── 02 ─────────────────────────────────────────────── */
  {
    ...
    body: [
      ...
      { h: 'Result', p: 'The tracker holds the bead through a full bell-shaped build of thousands of frames, and the loop drives speed up in the regions that can take it instead of running the whole part at the worst case. Faster prints, without giving up bond strength.' }
    ],
    gallery: [
  ...
  /* ── 03 ─────────────────────────────────────────────── */
  {
    ...
    body: [
      ...
      { h: 'Approach', p: 'I treated the legend as the primary detection target rather than an afterthought. A YOLOv8 model locates the legend blocks on the page; each swatch is sampled for its color and paired with its OCR\'d label, which gives you a per-district color→meaning dictionary. Only then does the pipeline walk the month grids, classify each day cell by swatch match, and emit structured events. SSIM, HOG and edge detection do the structural work of finding month blocks and day cells on documents that never agree on a layout.' },
...
/* ── experience timeline ─────────────────────────────── */
const TIMELINE = [
  { when: 'Since May 2026', what: 'Software Engineering Intern', where: 'Iridium Communications', kind: 'WORK', live: true },
  { when: 'Since Jan 2026', what: 'VP of New Scholars', where: 'Evans Scholars', kind: 'LEAD', live: true },
  { when: 'Jul to Sep 2025', what: 'AI Data Analytics Intern', where: 'Burbio', kind: 'WORK' },
  { when: 'Since Aug 2025', what: 'Numerai Prediction Model', where: 'ML@Purdue', kind: 'PROJECT', live: true },
  { when: 'Since Jan 2025', what: '3D Print Optimization', where: 'CMSC Research', kind: 'RESEARCH', live: true },
  { when: 'Aug 2024 to Jan 2025', what: 'Digital Twin Research', where: 'FLEX Lab', kind: 'RESEARCH' },
  { when: 'Since Aug 2024', what: 'Event Lead & Test Writer', where: 'Science Olympiad', kind: 'LEAD', live: true },
  { when: 'Since Aug 2024', what: 'B.S. Computer Science, Machine Intelligence', where: 'Purdue · Honors College · Chick Evans Scholar', kind: 'EDU', live: true }
];
